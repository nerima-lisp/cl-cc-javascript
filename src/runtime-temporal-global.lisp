...
;;; -----------------------------------------------------------------------
;;;  Per-namespace constructors — each takes initArgs object or positional,
;;;  mirroring how runtime-builtins-intl-*.lisp splits Intl per sub-object.
;;; -----------------------------------------------------------------------

(defun %js-make-temporal-instant-namespace ()
  (%js-make-object
   ...
   "compare" (lambda (a b)
               (%temporal-3way-compare (gethash "epochSeconds" a)
                                       (gethash "epochSeconds" b)))))

(defun %js-make-temporal-plain-date-namespace ()
  (%js-make-object
   ...
   "compare"  (lambda (a b)
                (%temporal-3way-compare
                 (%temporal-encode (gethash "year" a) (gethash "month" a) (gethash "day" a))
                 (%temporal-encode (gethash "year" b) (gethash "month" b) (gethash "day" b))))))

(defun %js-make-temporal-plain-time-namespace ()
  (%js-make-object
...
(defun %js-make-temporal-zoned-datetime-namespace ()
  (%js-make-object
   ...
   "from"     (lambda (str &optional _opts)
                ...
                (let* ((src  (if (%js-ht-p str) (gethash "toString" str) str))
                       (inst (%js-temporal-parse-instant src))
                       (ts   (gethash "epochSeconds" inst)))
                  (multiple-value-bind (s mn h d m y) (%temporal-decode ts)
  ...
  (%js-make-object
   ...
   "from"     (lambda (str-or-obj)
                (if (%js-ht-p str-or-obj)
                    ...
                    (%js-temporal-duration)))
   "compare"  (lambda (a b)
                (%temporal-3way-compare (%temporal-duration-to-seconds a)
                                        (%temporal-duration-to-seconds b)))))

(defun %js-make-temporal-plain-year-month-namespace ()
  (%js-make-object
...
(defun %js-make-temporal-global ()
  ...
  (%js-make-object
   "Now"            (%js-temporal-now)
   "Instant"        (%js-make-temporal-instant-namespace)
   "PlainDate"      (%js-make-temporal-plain-date-namespace)
   "PlainTime"      (%js-make-temporal-plain-time-namespace)
   "PlainDateTime"  (%js-make-temporal-plain-datetime-namespace)
   "ZonedDateTime"  (%js-make-temporal-zoned-datetime-namespace)
   "Duration"       (%js-make-temporal-duration-namespace)
   "PlainYearMonth" (%js-make-temporal-plain-year-month-namespace)
   "PlainMonthDay"  (%js-make-temporal-plain-month-day-namespace)))
