  ...
  (let* ((negative-p (and (plusp (length digits)) (char= (char digits 0) #\-)))
    ...
    (with-output-to-string (out)
      ...
      (loop for i below len
          ...
          (write-char #\, out)) (write-char (char body i) out)))))
...
(defun %js-number-format-compose (value options)
  (let* ((style (%js-to-string (%js-intl-option options "style" "decimal")))
        ...
        (max
          min-frac
          (%js-number-format-option-integer options "maximumFractionDigits" max-default)))
         (scaled-value
        (if (string= style "percent") (* (%js-to-number value) 100.0d0)
          (%js-to-number value)))
    ...
    (loop while (and
        (> (length fraction-string) min-frac)
        (char= (char fraction-string (1- (length fraction-string))) #\0))
          do (setf fraction-string (subseq fraction-string 0 (1- (length fraction-string)))))
    (let ((signed-integer
          (if negative-p (concatenate 'string "-" integer-string)
            integer-string)))
      (values
        (with-output-to-string (out)
          (write-string
            (if use-grouping (%js-number-format-group-integer signed-integer)
              signed-integer)
            out)
...
(defun %js-make-intl-number-format (&optional _locale options)
  ...
  (%js-make-object
    ...
    "formatToParts"
    (lambda (n)
      (multiple-value-bind (formatted integer-string fraction-string style) (%js-number-format-compose n options)
        (declare (ignore formatted))
        (let ((parts (list (%js-make-object "type" "integer" "value" integer-string))))
          (when (plusp (length fraction-string))
            (setf parts (append
                parts
                (list
                  (%js-make-object "type" "decimal" "value" ".")
                  (%js-make-object "type" "fraction" "value" fraction-string)))))
          (when (string= style "percent")
            (setf parts (append parts (list (%js-make-object "type" "percentSign" "value" "%")))))
