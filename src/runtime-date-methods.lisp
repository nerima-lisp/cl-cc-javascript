...
(defun %js-date-to-string (date)
  ...
  (if (%js-date-invalid-p date) "Invalid Date" (%js-date-to-iso-string date)))
...
(defun %js-date-to-json (date &optional key)
  "Date.prototype.toJSON() → ISO string, or null for an Invalid Date
(ignores KEY, per spec)."
  (declare (ignore key))
  (if (%js-date-invalid-p date) +js-null+ (%js-date-to-iso-string date)))
