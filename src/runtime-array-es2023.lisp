...
(defun %js-array-to-sorted (arr &optional compare-fn)
  ...
  (%js-array-stable-sort-undefined-last arr compare-fn))
...
(defun %js-array-to-spliced (arr start &optional (delete-count nil delete-count-supplied-p) &rest items)
  ...
  (let* ((n (length arr))
         ...
         (dc
        (if delete-count-supplied-p (max 0 (min (%js-array-to-integer delete-count) (- n st)))
          (- n st)))
         (before (subseq arr 0 st))
         (after (subseq arr (+ st dc)))
         (result (%js-make-vec)))
    (loop for x across before
          do (vector-push-extend x result))
    (dolist (item items)
      (vector-push-extend item result))
    (loop for x across after
          do (vector-push-extend x result))
    result))
    ...
    (%js-group-into
      (lambda (visit)
        (loop for i below (length arr)
              for el = (aref arr i)
              do (funcall visit (%js-to-string (%js-funcall key-fn el i arr)) el)))
      (lambda (key)
        (or
          (gethash key result)
          (setf (gethash key result) (%js-make-array)))))
    result))
  ...
  (let ((result (%js-make-map)))
    (%js-group-into
      (lambda (visit)
        (loop for i below (length arr)
              for el = (aref arr i)
              do (funcall visit (%js-funcall key-fn el i arr) el)))
      (lambda (key)
        (let ((bucket (%js-map-get result key)))
          (if (eq bucket +js-undefined+) (let ((fresh (%js-make-array)))
