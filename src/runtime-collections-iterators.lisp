...
(defun %js-iter-next (iter)
  ...
  (let* ((result
        (if (functionp iter) (funcall iter)
          (funcall (gethash "next" iter))))
         (ht-p (%js-ht-p result)))
    (values
      (if ht-p (gethash "value" result)
        result)
      (when ht-p
      ...
      (lambda ()
        (loop (when inner
            (multiple-value-bind (val done) (%js-iter-next inner)
              (unless done
                (return (list val)))
              (setf inner nil))) (multiple-value-bind (val done) (%js-iter-next iter)
            (when done
              (return :done))
            (let ((mapped (%js-funcall fn val)))
              (setf inner (if (%js-vec-p mapped) (%js-vec-to-iter mapped)
                  mapped)))))))))
...
BODY runs for each element; DONE-RESULT is returned when the iterator exhausts."
  (let ((done (gensym "done")))
    `(loop (multiple-value-bind (,var ,done) (%js-iter-next ,iter)
        (when ,done
          (return ,done-result))
        ,@body))))
...
(defun %js-iterator-reduce (iter fn &optional (init +js-undefined+))
  (let ((acc init)
        (first-p (eq init +js-undefined+)))
    (%js-doiter
      (val iter acc)
      (if first-p (setf acc val
              first-p nil)
...
(defun %js-iterator-to-array (iter)
  (let ((result (make-array 0 :element-type t :adjustable t :fill-pointer 0)))
    (%js-doiter (val iter result) (vector-push-extend val result))))

(defun %js-iterator-for-each (iter fn)
  (%js-doiter (val iter +js-undefined+) (%js-funcall fn val)))
...
(defun %js-iterator-every (iter fn)
  (%js-doiter
    (val iter t)
    (unless (%js-truthy (%js-funcall fn val))
      (return))))

(defun %js-iterator-find (iter fn)
  (%js-doiter
    (val iter +js-undefined+)
    (when (%js-truthy (%js-funcall fn val))
      (return val))))
      ...
      (lambda ()
        (loop (when current
            (multiple-value-bind (val done) (%js-iter-next current)
              (unless done
                (return (list val)))
              (setf current nil))) (unless sources
            (return :done)) (setf current (pop sources)))))))
...
(defparameter *%js-iterator-method-names* '(("map" . %js-iterator-map)
