...
(defun %js-strip-trailing-dot (s)
  ...
  (if (and (plusp (length s)) (char= (char s (1- (length s))) #\.)) (subseq s 0 (1- (length s)))
    s))

(defun %js-strip-pre-exp-dot (s)
  "Remove a lone '.' immediately before the exponent marker."
  (let ((pos (or (search ".e" s) (search ".E" s))))
    (if pos (concatenate 'string (subseq s 0 pos) (subseq s (1+ pos)))
      s)))

(defun %js-number-to-precision (x p)
  "JS Number.prototype.toPrecision(P): render X to P significant digits."
  (cond
    ((or (< p 1) (> p 100)) (princ-to-string x))
    ((zerop x)
      (if (= p 1) "0"
        (format nil "0.~A" (make-string (1- p) :initial-element #\0))))
    (t
      (let* ((neg (minusp x))
             (ax (abs (coerce x 'double-float)))
             (e (floor (log ax 10d0))))
        (loop while (>= (/ ax (expt 10d0 e)) 10d0)
              do (incf e))
        (loop while (< (/ ax (expt 10d0 e)) 1d0)
              do (decf e))
        (let ((body
              (if (or (< e -6) (>= e p)) (%js-strip-pre-exp-dot (format nil "~,v,,,,,'eE" (1- p) ax))
                (%js-strip-trailing-dot (format nil "~,vF" (max 0 (- p 1 e)) ax)))))
          (if neg (concatenate 'string "-" body)
            body))))))
...
(Function.prototype.name, RegExp fallback accessors), not methods."
  (let ((entry (assoc key table :test #'string=)))
    (if entry (funcall (cdr entry) obj)
      +js-undefined+)))
