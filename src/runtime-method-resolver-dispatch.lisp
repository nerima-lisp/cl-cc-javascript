...
(define-js-type-resolver
  %js-resolve-array-method
  *js-array-method-table*
  "length"
  (coerce (length obj) 'double-float))

(define-js-type-resolver
  %js-resolve-string-method
  *js-string-method-table*
  "length"
  (coerce (length obj) 'double-float))

(define-js-type-resolver
  %js-resolve-map-method
  *js-map-method-table*
  "size"
  (coerce (%js-map-size obj) 'double-float))

(define-js-type-resolver
  %js-resolve-set-method
  *js-set-method-table*
  "size"
  (coerce (%js-set-size obj) 'double-float))
...
(defparameter *js-object-fallback-method-table* (list
    (cons
      "hasOwnProperty"
      (lambda (obj)
        (lambda (k)
          (nth-value 1 (gethash (%js-to-string k) obj)))))
    (cons
      "toString"
      (lambda (_obj)
        (declare (ignore _obj))
...
(define-js-type-resolver
  %js-resolve-regexp-method
  nil
  "source"
  (js-regexp-source obj)
  "flags"
  (js-regexp-flags obj)
  "global"
  (js-regexp-global-p obj)
  "ignoreCase"
  (js-regexp-ignore-case-p obj)
  "multiline"
  (js-regexp-multiline-p obj)
  "lastIndex"
  (coerce (js-regexp-last-index obj) 'double-float)
  "test"
  (let ((re obj))
    (lambda (str)
      (%js-regex-test re str)))
  "exec"
  (let ((re obj))
    (lambda (str)
...
(define-js-type-resolver %js-resolve-number-method *js-number-method-table*)

(define-js-type-resolver
  %js-resolve-symbol-method
  *js-symbol-method-table*
  "description"
  (%js-symbol-description obj))

(define-js-type-resolver
  %js-resolve-weak-ref-method
  nil
  "deref"
  (lambda ()
    (%js-weak-ref-deref obj)))

(defparameter *js-function-method-table* (list
    (cons
      "bind"
      (lambda (fn)
        (lambda (this-arg &rest partial-args)
          (lambda (&rest args)
            (apply #'%js-funcall fn (cons this-arg (append partial-args args)))))))
    (cons
      "call"
      (lambda (fn)
        (lambda (this-arg &rest args)
          (apply #'%js-funcall fn (cons this-arg args)))))
    (cons
      "apply"
      (lambda (fn)
        (lambda (this-arg args-array)
          (apply
            ...
            (cons
              this-arg
              (when (%js-vec-p args-array)
                (coerce args-array 'list)))))))
...
(defparameter *js-type-dispatch-table* (list
    (cons #'js-promise-p #'%js-resolve-promise-method)
