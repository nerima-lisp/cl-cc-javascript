...
(defparameter *js-array-method-table* (list
    (cons "push" #'%js-array-push)
    (cons "pop" #'%js-array-pop)
    (cons "shift" #'%js-array-shift)
    (cons "unshift" #'%js-array-unshift)
    (cons "map" #'%js-array-map)
    (cons "forEach" #'%js-array-for-each)
    (cons "filter" #'%js-array-filter)
    (cons "reduce" #'%js-array-reduce)
    (cons "reduceRight" #'%js-array-reduce-right)
    (cons "find" #'%js-array-find)
    (cons "findIndex" #'%js-array-find-index)
    (cons "some" #'%js-array-some)
    (cons "every" #'%js-array-every)
    (cons "includes" #'%js-array-includes)
    (cons "indexOf" #'%js-array-index-of)
    (cons "lastIndexOf" #'%js-array-last-index-of)
    (cons "join" #'%js-array-join)
    (cons "slice" #'%js-array-slice)
    (cons "splice" #'%js-array-splice)
    (cons "concat" #'%js-array-concat)
    (cons "reverse" #'%js-array-reverse)
    (cons "sort" #'%js-array-sort)
    (cons "flat" #'%js-array-flat)
    (cons "flatMap" #'%js-array-flat-map)
    (cons "fill" #'%js-array-fill)
    (cons "copyWithin" #'%js-array-copy-within)
    (cons "entries" #'%js-array-entries)
    (cons "keys" #'%js-array-keys)
...
(defparameter *js-string-method-table* (list
    (cons "slice" #'%js-string-slice)
    (cons "indexOf" #'%js-string-index-of)
    (cons "lastIndexOf" #'%js-string-last-index-of)
    (cons "includes" #'%js-string-includes)
    (cons "startsWith" #'%js-string-starts-with)
    (cons "endsWith" #'%js-string-ends-with)
    (cons "split" #'%js-string-split-regex)
    (cons "replace" #'%js-string-replace-regex)
    (cons "replaceAll" #'%js-string-replace-all-regex)
    (cons "padStart" #'%js-string-pad-start)
    (cons "padEnd" #'%js-string-pad-end)
    (cons "at" #'%js-string-at)
    (cons "repeat" #'%js-string-repeat)
    (cons "charAt" #'%js-string-char-at)
    (cons "charCodeAt" #'%js-string-char-code-at)
    (cons "concat" #'%js-string-concat)
    (cons "match" #'%js-string-match-regex)
    ...
    (cons "trim" #'%js-string-trim)
    (cons "trimStart" #'%js-string-trim-start)
...
(defparameter *js-set-method-table* (list
    (cons "add" #'%js-set-add)
...
(defparameter *js-map-method-table* (list
    (cons "set" #'%js-map-set)
    (cons "get" #'%js-map-get)
    (cons "has" #'%js-map-has)
...
(defparameter *js-weak-map-method-table* (list
    (cons "set" #'%js-weak-map-set)
    ...
    (cons
      ...
      (lambda (n &optional (radix 10))
        (let ((r (%js-int-arg-or-default radix 10))
          ...
          (if (= r 10) (%js-to-string n)
            (string-downcase (format nil "~vR" r ni))))))
    (cons
      "toPrecision"
      (lambda (n prec)
        (if (eq prec +js-undefined+) (princ-to-string (%js-to-number n))
    ...
    (cons
      ...
      (lambda (n &optional digits)
        (let* ((v (%js-to-number n))
               (d
              (if (or (null digits) (eq digits +js-undefined+)) 6
                (truncate (%js-to-number digits)))))
...
(defparameter *js-symbol-method-table* (list
    (cons "toString" #'%js-symbol-to-string)
    (cons
      "valueOf"
      (lambda (s)
        s)))
