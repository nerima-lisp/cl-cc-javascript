...
(defun %js-unsigned-shift-right (a b)
  ...
  (ash (%js-to-int32 a) (- (mod (logand (%js-to-int32 b) #x1F) 32))))
...
(defun %js-postfix-inc (val)
  val)

(defun %js-postfix-dec (val)
  val)

(defun %js-prefix-inc (val)
  (1+ (%js-to-number val)))

(defun %js-prefix-dec (val)
  (1- (%js-to-number val)))

;;; ─── Regex literals ───────────────────────────────────────────────────────────
;;; Full RegExp implementation spans runtime-regex-combinators.lisp,
;;; runtime-regex.lisp, and runtime-regex-api.lisp (all loaded before this
;;; file). %js-make-regex, the native NFA-based engine's public constructor,
;;; is defined in runtime-regex-api.lisp.
...
(defun %js-bigint (x)
  ...
  (cond
    ...
    ((stringp x)
      (handler-case (%make-js-bigint (parse-integer x :junk-allowed t))
        (error ()
          (error "Cannot convert ~A to BigInt" x))))
    (t (%make-js-bigint 0))))
...
(defun %js-bigint-val (x)
  (if (js-bigint-p x) (js-bigint-value x)
    (truncate x)))
