...
(defun %js-zip-normalize-iterable (value)
  (cond
    ...
    ((js-set-p value) (%js-vec-to-iter (coerce (%js-set-keys value) 'vector)))
    ((%js-vec-p value) (%js-vec-to-iter value))
    ((%js-ht-p value)
      (cond
        ((gethash "next" value) value)
    ...
    ((listp value) (%js-vec-to-iter (coerce value 'vector)))
    (t (%js-zip-type-error "Iterator.zip expects iterable objects"))))

(defun %js-zip-normalize-options (options)
  (cond
    ((or (null options) (eq options +js-undefined+)) (%js-make-object))
    ((%js-ht-p options) options)
    (t (%js-zip-type-error "Iterator.zip options must be an object"))))

(defun %js-zip-collect-source-iterators (iterables)
  (let ((outer (%js-zip-normalize-iterable iterables))
        (sources nil))
    (loop (multiple-value-bind (item done) (%js-iter-next outer)
        (when done
...
(defun %js-zip-finish-step (all-done-p some-done-p mode row-or-result)
  "The terminal decision shared by one %JS-ITERATOR-ZIP/-ZIP-KEYED step, once
every source has been asked for its next value: :DONE when every source is
exhausted, a type error under \"strict\" mode when sources finished
unevenly, :DONE (silently truncating) under \"shortest\", or the collected
ROW-OR-RESULT (already padded under \"longest\" by the caller) wrapped as the
one-element list %JS-MAKE-CL-ITERATOR's step function expects."
  (cond
    (all-done-p :done)
    ((and some-done-p (string= mode "strict"))
     (%js-zip-type-error "Iterator.zip strict mode requires equal-length iterables"))
    ((and some-done-p (string= mode "shortest")) :done)
    (t (list row-or-result))))
  ...
  (let ((options (%js-zip-normalize-options options)))
    (multiple-value-bind (mode padding-option) (%js-zip-read-mode options)
      (let* ((sources (%js-zip-collect-source-iterators iterables))
        ...
        (%js-make-cl-iterator
          (lambda ()
            (let ((row (make-array source-count :element-type t :initial-element +js-undefined+))
                  (all-done-p t)
                  (some-done-p nil))
              ...
              (%js-zip-finish-step all-done-p some-done-p mode row))))))))
            ...
            (let ((result (%js-object-create +js-null+))
              ...
              (loop for key in keys
                  ...
                  (if done (progn
                      (setf some-done-p t)
                      (when (string= mode "longest")
                        (setf (gethash key result) (gethash key padding +js-undefined+))))
                    (progn
                      (setf all-done-p nil)
                      (setf (gethash key result) value)))))
              (%js-zip-finish-step all-done-p some-done-p mode result))))))))
