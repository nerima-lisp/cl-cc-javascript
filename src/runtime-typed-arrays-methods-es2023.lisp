...
(defun %js-ta-to-sorted (ta &optional compare-fn)
  ...
  (let* ((n (js-ta-length ta))
         ...
         (sorted
        (sort
          (coerce (copy-seq (js-ta-buffer ta)) 'list)
          (if compare-fn (lambda (a b)
              (minusp (%js-to-number (%js-funcall compare-fn a b))))
            #'<))))
    (loop for v in sorted
          for i from 0
          do (setf (aref new-buf i) v))
    (%js-ta-clone-with-buffer ta new-buf)))
...
to also return the found value (find/findLast return elements, not indices)."
  (let* ((not-found
        (if (eq return :value) '+js-undefined+
          '-1.0d0))
         (found-form
        (if (eq return :value) 'v
          '(coerce i 'double-float)))
         (loop-clause
        (if (eq direction :forward) `(dotimes (i (js-ta-length ta) ,not-found)
...
(define-js-ta-find %js-ta-find :forward :value "TypedArray.prototype.find().")
...
pattern — the TypedArray analogue of DEFINE-JS-ARRAY-PREDICATE-TEST."
  `(defun ,name (ta fn)
    ...
    (loop for i below (js-ta-length ta)
          for element = (coerce (aref (js-ta-buffer ta) i) 'double-float) ,clause (%js-truthy (%js-funcall fn element i ta))
          return ,early-result
          finally (return ,final-result))))

(define-js-ta-predicate-test
  %js-ta-every
  unless
  nil
  t
  ...
  `(defun ,name (ta)
    ...
    (let ((i (list 0))
      ...
      (%js-make-object
        ...
        (lambda ()
          (if (< (car i) n) (prog1
              (%js-make-object "value" ,value-form "done" nil)
              (incf (car i)))
            (%js-make-object "value" +js-undefined+ "done" t)))
        ,@(when self-iterator
          `("@@iterator"
            (lambda ()
              (gethash "@@iterator" (,name ta)))))))))

(define-js-ta-iterator
  %js-ta-values
