...
(defun %js-reflect-get (target key &optional receiver)
  (if (%js-proxy-object-p target) (%js-proxy-get target key (or receiver target))
    (%js-get-prop target key)))

(defun %js-reflect-set (target key value &optional receiver)
  (if (%js-proxy-object-p target) (%js-proxy-set target key value (or receiver target))
    (progn
...
(defun %js-object-data-property-descriptor (obj value)
  (%js-make-object
    ...
    t
    "configurable"
    (not (%js-object-sealed-p obj))))

(defun %js-object-accessor-property-descriptor (obj key)
  (let* ((descriptor
        (%js-make-object "enumerable" t "configurable" (not (%js-object-sealed-p obj))))
         (getter-key (%js-object-descriptor-slot-key "__get_" key))
         (setter-key (%js-object-descriptor-slot-key "__set_" key)))
...
(defun %js-reflect-define-property (target key descriptor)
  (cond
    ((%js-proxy-object-p target) (%js-proxy-define-property target key descriptor))
    (t (%js-object-apply-property-descriptor target key descriptor))))
...
(defun %js-reflect-set-prototype-of (target proto)
  (when (%js-ht-p target)
    (let ((before (%js-object-get-prototype-of target)))
      ...
      (or (eq before proto) (eq (%js-object-get-prototype-of target) proto)))))
...
(defun %js-object-get-own-property-descriptor (obj key)
  (cond
    ((%js-proxy-object-p obj) (%js-proxy-get-own-property-descriptor obj key))
    ((%js-ht-p obj)
      (let ((k (%js-to-property-key key)))
        (multiple-value-bind (val found) (gethash k obj)
          (cond
            (found (%js-object-data-property-descriptor obj val))
        ...
        (maphash
          (lambda (k v)
            (unless (%js-internal-key-p k)
              (setf (gethash k result) (%js-object-data-property-descriptor obj v))))
          obj)
        (maphash
          (lambda (k v)
            (declare (ignore v))
            (let ((property-name (%js-object-accessor-property-name k)))
              (when (and property-name (not (nth-value 1 (gethash property-name result))))
                (setf (gethash property-name result) (%js-object-accessor-property-descriptor obj property-name)))))
          obj)))
    result))
...
(defun %js-proxy-call-trap (proxy name &rest args)
  (let ((trap (%js-proxy-trap proxy name)))
    (if trap (values (apply #'%js-funcall trap args) t)
...
(defun %js-proxy-get (proxy key &optional receiver)
  (let ((target (%js-proxy-target proxy)))
    (multiple-value-bind (result trapped) (%js-proxy-call-trap proxy "get" target (%js-to-string key) (or receiver proxy))
      (if trapped result
        (%js-get-prop target key)))))

(defun %js-proxy-set (proxy key value &optional receiver)
  (let ((target (%js-proxy-target proxy)))
    (multiple-value-bind (result trapped) (%js-proxy-call-trap
        proxy
        "set"
        target
        (%js-to-string key)
...
(defun %js-proxy-has (proxy key)
  (let ((target (%js-proxy-target proxy)))
    (multiple-value-bind (result trapped) (%js-proxy-call-trap proxy "has" target (%js-to-string key))
      (if trapped (%js-truthy result)
        (%js-in key target)))))

(defun %js-proxy-delete-property (proxy key)
  (let ((target (%js-proxy-target proxy)))
    (multiple-value-bind (result trapped) (%js-proxy-call-trap proxy "deleteProperty" target (%js-to-string key))
      (if trapped (%js-truthy result)
        (%js-delete target key)))))

(defun %js-proxy-own-keys (proxy)
  (let ((target (%js-proxy-target proxy)))
    (multiple-value-bind (result trapped) (%js-proxy-call-trap proxy "ownKeys" target)
      (if trapped result
